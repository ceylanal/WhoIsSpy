...
export default function RevealPage() {
  ...
  const handleNextPlayer = () => {
    ...
    // Wait for slide-out animation to complete (400ms duration)
    setTimeout(() => {
      ...
      if (updated.phase === 'PLAY') {
        ...
        setGameState(updated);
        // Switch to slide-in animation from the left
        setAnimationState('slide-in');
        
        // Wait for slide-in animation to complete (400ms duration)
        setTimeout(() => {
  ...
  const cardBack = (gameState.isCardRevealed || animationState === 'slide-out') ? (
    currentPlayer.isSpy ? (
      <div className="flex flex-col items-center justify-center text-center space-y-6">
    ...
    <main className="flex-1 flex flex-col p-6 max-w-md mx-auto w-full relative z-10 items-center justify-between pb-12">
      ...
      <div className="my-8 flex justify-center w-full">
        <FlipCard
          ...
          front={cardFront}
          back={cardBack}
          disableTransition={animationState !== 'idle'}
          className={
